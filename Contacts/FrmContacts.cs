/**
* Application pour gérer une liste de contact
* auteur: RICHARD Forent
* date: 21 / 10 / 2020
*/
using System;
...
namespace Contacts
{
    ...
    public partial class frmContacts : Form
    {
        ...
        private void FinAjout()
        {
            ...
            // Bouton radio particulier sélectionné
        ...
        private bool RecherchePrenom(String prenom)
        {
            for (int k = 0; k < lstContact.Items.Count; k++)
            {
               ...
               ((Particulier)contact).getPrenom().ToLower().Contains(prenom.ToLower()))
                {
                    lstContact.SelectedIndex = k;
                    return true;
                }
        ...
        /// <summary>
        /// Supprime les zone de texte des recherches
        /// </summary>
        private void SupRecherche()
        {
            txtNomRech.Text = "";
            txtPrenomRech.Text = "";
            txtTelRech.Text = "";
        ...
        private void BtnAjouter_Click(object sender, EventArgs e)
        {
            ...
            if (!txtNom.Text.Equals("") && !txtTel.Text.Equals("") && ((rdbParticulier.Checked && !txtPrenom.Text.Equals("")) || rdbProfessionnel.Checked))
            {
                // créer le contact et l'ajouter dans la collection
                Contact nouveauContact;
                if (rdbParticulier.Checked)
                {
                    nouveauContact = new Particulier(txtNom.Text, txtPrenom.Text, txtTel.Text,
                    imgPhoto.Image);
                }
                else
                {
        ...
        private void lstContact_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            // Crée un pinceau en lui affectant une couleur
            Brush myBrush = new SolidBrush(couleur);
            // Permet de dessiner le texte de l'item correspondant
            e.Graphics.DrawString(ligne, e.Font, myBrush, e.Bounds, StringFormat.GenericDefault);
            //Dessine le cadre de focus autour de la ListBox, si elle a le focus.
            e.DrawFocusRectangle();
        }

        private void txtNomRech_TextChanged(object sender, EventArgs e)
        {
            // Vide les autres zones de recherche
            txtPrenomRech.Text = "";
            txtTelRech.Text = "";
            // lance la recherche
            RechercheNom(txtNomRech.Text);
        }

        private void txtPrenomRech_TextChanged(object sender, EventArgs e)
        {
            // Vide les autres zones de recherche
            txtNomRech.Text = "";
            txtTelRech.Text = "";
            // lance la recherche
            RecherchePrenom(txtPrenomRech.Text);
        }

        private void txtTelRech_TextChanged(object sender, EventArgs e)
        {
            // Vide les autres zones de recherche
            txtPrenomRech.Text = "";
            txtNomRech.Text = "";
            // lance la recherche
            RechercheTel(txtTelRech.Text);
        }
